...
1.  ¿Los parámetros corresponden al ensamblaje de un genoma haploide o diploide?
    ...
    Se corresponde a un genoma haploide. El parámetro `–f`indica la cobertura esperada en una región homocigota, pero al haber indicado que es cero se asume que no hay regiones homocigotas porque es un genoma haploide.
...
2.  ¿Qué nivel de eliminación de duplicaciones hemos aplicado? (El paso de eliminar duplicaciones sirve para que las zonas de alta heterocigosidad no sean ensambladas como contigs diferentes).
    ...
    Esto se indica con el parámetro `-r`, pero como elimina repeticiones en zonas de alta heterocigosidad y tenemos un genoma haploide, no hay zonas heterocigotas, por loque no es necesario.

3.  Si tuvieramos lecturas ultra-largas, ¿con qué opción o argumento añadiríamos esa información?

    Los parámetros sirven para añadir información sobre lecturas ultra-largas:
...
1.  ¿Dónde están las secuencias que componen el ensamblaje?
    ...
    En el archivo `At4.bp.p_ctg.gfa` esta en la carpeta `asm1`, este archivo contiene el ensamblaje de los contigs.
...
3.  ¿Qué información tiene los archivos `.bed`?
    ...
    Estos archivos contiene las coordenadas de las regiones de poca calidad.

4.  ¿Cuál es la cobertura media estimada de las secuencias homocigotas?

    En el archivo `himiasm.log` se especifica que la cobertura media estimada para las secuencias homocigotas es 12.

5.  ¿Y la de las heterocigotas?

    Como el genoma era haploide no podemos estimar la cobertura de las regiones heterocigotas. En el archivo `hifiasm.log` se representa las curvas de frecuenci de k-meros, en las que observa un solo pico, esto nos confirma que no hay regiones heterocigotas.
:::
...
· `-l brassicales`: Indica el orden al que pertenece Arabidopsis thaliana. Lo que hace es indicar al programa en qué grupo debe buscar los genes para así poder evaluaf la calidad del ensamblaje.
