...
La lista tiene una longitud de 42 secuencias con un alineamiento de longitud 16603pb. Con `names(mtdna)` podemos extarer el nombre de las secuencias y la especie de las que hemos extraído.
:::
...
Para saber si esta enraizado el árbol

```{r}
is.rooted(mt_nj)
```

Para saber cómo identificar los nodos internos, puedes invocar `nodelabels()` después de haber usado `plot(mt_nj)`.
...
La función add.scale.bar(mt_nj)= indica la cantidad de cambio evolutivo (distancia genética), es decir, nos ayuda a entender cuanto difieren las secuencias.

Y la función axisPhylo(mt_nj)=añade un eje con escala numérica al árbol mostrando valores acumulados de distancia desde la raiz.
:::
...
El árbol `mt_nj` no está enraizado (c omo hemos comprobado antes). Tenemos un par de opciones para enraizarlo. Una opción es `root()`, función en la que debemos especificar o bien el *outgroup* o bien el **nodo** interno en el que queremos situar la raíz. Por ejemplo, en mi caso, el nodo 75 parece ser el ancestro entre gorilas y chimpancés, y por tanto también es ancestro de los humanos. Por tanto:
...
A continuación se escoge la posición de la raíz que optimiza la regresión entre distancia de raíz a puntas y edades de las puntas ([Rambaut et al. 2016](https://aulavirtual.uv.es/pluginfile.php/6009204/mod_resource/content/3/guio-es.html#ref-Rambaut2016)).
...
NO. Hay ramas que se han acumulado cmabios que deberían haber quedado más atrás. rtt lo que hace es buscar la mejor correlacion entre las ramas y los tiempos, pero es dificil hacerlo encajar. Por ejemplo: parece que los neardentales o otras especies estan más cerca que los modernos.

-   ¿Qué secuencias crees que se ajustan menos a su supuesta edad?

Los neardentales y y los denisovanos

-   ¿Qué razones puede haber para que la edad de una secuencia no se corresponda con la cantidad de cambios que ha acumulado?

Podemos pensar que la datación no es correcta y no tengan su edad verdadera por un fallo de la datación. Posterioremente vamos a eliminar la secuencia del gorila porque es muy antigua y puede generar controversias. Además de eliminar algunos pares o trios de secuencias que son idénticos, para quedarnos solo con uno del grupo. Todo esto para facilitar que todo salga bien en el programa.
:::
...
De los humanos a los chimpaces hay una distancia de 0,09104153 y entre humanos y neandertales es de 0,01214282. Es 7,497561 veces mayor.
...
Al ver una relación lineal todo lo que coincida en la diagonal es que son prácticamente idénticos. Por lo que podemos decir que las distancias entre los valores esperados y los observados son similares.
:::
...
Partimos de la de 37porque antes hemos eliminado los duplicamos
